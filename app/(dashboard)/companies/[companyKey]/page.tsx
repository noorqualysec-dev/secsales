...
import { ArrowLeft, BadgeCheck, Building2, Calendar, Mail, Phone, Sparkles, UserRound, Users } from "lucide-react";
...
export default function CompanyDetailPage() {
  ...
  return (
    <div className="space-y-8">
      <Link
        href="/companies"
        className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-bold text-slate-600 transition hover:border-indigo-200 hover:text-indigo-700"
      >
              ...
              <div key={member.contactId} className="rounded-[1.75rem] border border-slate-200 bg-[linear-gradient(135deg,_#fff,_#f8fbff)] p-5">
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-slate-600 transition hover:border-indigo-200 hover:text-indigo-700"
                  >
            ...
            <div className="mt-6 space-y-4">
              {company.leads.map((lead) => (
                <Link
                  key={lead._id}
                  href={`/leads/${lead._id}`}
                  className="block rounded-[1.5rem] border border-slate-200 p-4 transition hover:border-indigo-200 hover:bg-slate-50"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-base font-black tracking-tight text-slate-900">{lead.firstName} {lead.lastName}</p>
